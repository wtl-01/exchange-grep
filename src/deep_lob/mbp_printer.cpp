// Compile:
// g++ -Wextra -O2 -o mbp_parser mbp_printer.cpp order_book_manager.cpp order_book.cpp -lpcap -std=c++20 -I include
// use -g for debug symbols in gdb
// Run:
// ./mbp_parser ../../md/md-cfe-tsp05-eth7-d-2024-41-ok.pcap
...
struct MetaDataManager {
    ...
    string format_line(L2RecordEntry& l2) {
        char buffer[1600];  // Adjust size as needed

        // Bids as a comma-separated list, padded by [].
        // Print up to 10 levels, fill with 0 if not enough levels
        // for each level, print size, and number of orders as (size, num_orders)
        // so it would look like: [(1000, 5), (990, 3), ...]

        std::ostringstream bids_stream;
        // enclose the stream in "[ ... ]"
        bids_stream << "\"";
        bids_stream << "[";
        for (int i = 0; i < 10; ++i) {
            if (i < l2.bids.size()) {
                bids_stream << "(" << l2.bids[i].qty << ", " << l2.bids[i].numOrders << ")";
            } else {
                bids_stream << "(0, 0)";
            }
            if (i < 9) {
        ...
        // FORMAT BID STREAM AS CSTRING
        std::string bids_str = bids_stream.str();

        std::ostringstream asks_stream;
        asks_stream << "\"";
        asks_stream << "[";
        for (int i = 0; i < 10; ++i) {
            if (i < l2.asks.size()) {
                asks_stream << "(" << l2.asks[i].qty << ", " << l2.asks[i].numOrders << ")";
            } else {
                // cerr << "Filling ask level " << i << " with (0,0)" << endl;
                asks_stream << "(0, 0)";
            }
            if (i < 9) {
                asks_stream << ", ";
        ...
        asks_stream << "]";
        asks_stream << "\"";
        std::string asks_str = asks_stream.str();

        int len = snprintf(buffer, sizeof(buffer),
                           "%s,%" PRIu64 ",%" PRIu64 ",%c,%s,%c,%.3f,%.3f,%.3f,%s,%s \n",
                           l2.srcTime.c_str(),
                           l2.pktSeqNum,
                           l2.msgSeqNum,
                           l2.msgType,
                           ...
                           l2.tradingStatus,
                           l2.ticksize,
                           static_cast<double>(l2.best_bid_price) * 10e-3,
                           static_cast<double>(l2.best_ask_price) * 10e-3,
                           bids_str.c_str(),
                           asks_str.c_str()
        );
    ...
    string return_header() {
        return "srcTime,pktSeqNum,msgSeqNum,msgType,Symbol,tradingStatus,tickSize,bestBidPx,bestAskPx,bids,asks\n";
    }
    
    bool write_outfile(string line) {
        if (outfile.is_open()) {
            outfile << line;
            return true;
        }
    ...
    bool process_L2_record(L2RecordEntry& l2) {
        string line = format_line(l2);
        return write_outfile(line);
    }
...
string getTimestampStringOld(const string &Date, uint32_t timeInSecs, uint32_t timeOffset)
{
    ...
    case 0x22: // AddOrderShort
    {
        ...
        */
        ...
        if (msg_type == 0x21)
        {
            ...
            // TODO: ADD Logic for printing out L2
            L2RecordEntry l2_entry = mdm.obm.get_L2_record(ticker, mdm.symbol_to_readable_dict);
            ...
            char status = es == ExchangeStatus::S ? 'S' : es == ExchangeStatus::T ? 'T' : es == ExchangeStatus::Q ? 'Q' : 'I';
            char oper = 'A'; // ADD

            // PRINT UPDATE TO ENTRY STRUCT
            l2_entry.pktSeqNum = pktSeqNum;
            l2_entry.msgSeqNum = msgSeqNum;
            l2_entry.msgType = oper;
            l2_entry.srcTime = srcTime;
            l2_entry.tradingStatus = status;

            // Write out L2 record
            mdm.process_L2_record(l2_entry);
...
void process_day_packets(const std::vector<PacketData>& day_packets, pair<string, string> cdtrf, uint32_t tsmp) {
    ...
    string output_filename = "CFE_" + loc_mdm.currentDate + "_Level2.csv";
    // open file for writing
    loc_mdm.outfile = std::ofstream(output_filename);
    if (!loc_mdm.outfile.is_open()) {
        cerr << "Error opening output file: " << output_filename << endl;
        exit(1);
    }
    // Write CSV header
    loc_mdm.write_outfile(loc_mdm.return_header());
    ...
    cerr << "Records " << loc_mdm.currentDate << " Has been completely processed." << endl;

    // Output the CSV here for the day
    loc_mdm.outfile.close();
    cerr << "Output for day " << loc_mdm.currentDate << " written to " << output_filename << endl;
    

    // TODO: Add logic to write out the L2 data to CSV
}
...
void printUsage(const char* programName) {
    std::cerr << "Usage: " << programName << " <pcap_file> [-out output_file] [-timeit] --help\n"
              ...
              << "\nOptions:\n"
              << "  -o, --out <file>   Specify output CSV file (default: L2_2024-41.csv)\n"
...
int main(int argc, char *argv[])
{
    ...
    std::string outputFileName = "L2_2024-41.csv";
    ...
    cerr << "Processing PCAP file: " << pcapFileName << endl;
    ...
    //cout << curr_dateref.first << " " << curr_dateref.second << endl;
    // TODO: Initialize curr_dateref because its now "", ""
    // Hard code for now
    string init_date = "20241006";
    string init_tradedate = "20241007";
    curr_dateref = make_pair(init_date, init_tradedate);
    // lets see if this thing breaks
    cout << "First Day: " <<curr_dateref.first << " " << curr_dateref.second << endl;
    ...
    pcap_close(pcap);
    cerr << "Total days of data found: " << days_packets.size() << endl;

    // TODO: we can preconfigure application parameters to preallocate RAM
    // TODO: dev/shm any file u put here will be temporarily persisted in the fs, dautomatically stored to memory, not written to disc
    sw2.Stop();
    sw.Start();
    ...
    for (size_t i = 0; i < days_packets.size(); ++i) {
        threads.emplace_back([&, i]() {
            cerr << "Processing day " << i + 1 << endl;
            process_day_packets(days_packets[i], daterefs[i], timestamp_ref[i]);
        });
    }
    for (auto& th : threads) {
        th.join();
