...
struct MetaDataManager {
        ...
        //vector<BBORecordEntry> BBO_Entries;
    ...
    void cout_print_bbo(const uint64_t pktSeqNum, uint64_t msgSeqNum, const std::string& ticker, char oper, const BestBidOffer& bbo, const std::string& srcTime, const std::string& trade_status) {
        ...
        // change the 6f to 2f formatting
        int len = snprintf(buffer, sizeof(buffer), "%s,%" PRIu64 ",%" PRIu64 ",%c,%s,%.2f,%u,%.2f,%u,%s\n",
                        srcTime.c_str(),
                        pktSeqNum,
                        msgSeqNum,
                        oper,
...
//             BBO_Entries.emplace_back(BBORecordEntry{
//                srcTime,                   // Source timestamp
//                pktSeqNum,                 // Packet Sequence Number
//                msgSeqNum,                 // Message Sequence Number
//                oper,                      // Operation character
//                ticker,                    // Human-readable ticker name
//                static_cast<double>(bbo.best_bid_price)*10e-3,          // Best bid price
//                bbo.best_bid_amount,         // Best bid amount
//                static_cast<double>(bbo.best_ask_price)*10e-3 ,          // Best ask price
//                bbo.best_ask_amount,         // Best ask amount
...
//void cout_print_bbo_v1(const uint64_t pktSeqNum, uint64_t msgSeqNum, string ticker, char& oper, BestBidOffer bbo, string srcTime, string trade_status, MetaDataManager mdm)
//{
//        ostringstream oss;
//        oss << srcTime << ","
//                << pktSeqNum << ","
//                << msgSeqNum << ","
//                << oper << ","
//                << mdm.symbol_to_readable_dict[ticker]["name"] << "," // Human Readable Ticker
...
//        mdm.BBO_Entries.push_back(oss.str());
//}
...
void process_day_packets(const std::vector<PacketData>& day_packets, std::vector<string>& day_bbo_entries, pair<string, string> cdtrf, uint32_t tsmp) {
        ...
        //loc_mdm.time_offset = timestamp_ref_curr;
...
int main(int argc, char *argv[])
{
        ...
        // TODO: Emplace zero copying: avoid copy as much as possible.
        // TODO: ZERO copy: what if you never copy the code? access is not copy
